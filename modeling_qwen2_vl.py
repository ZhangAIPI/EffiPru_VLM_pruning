...
class Qwen2VLAttention(nn.Module):
    ...
    def __init__(self, config: Qwen2VLConfig, layer_idx: Optional[int] = None):
        ...
        self.offset = 15 # system prompt token len
    ...
    def forward(
        ...
        if self.layercut and self.layer_idx>=self.layercut_idx:
                if attn_weights.shape[2]>image_token_num:
                    attn_weights[:,:,image_token_num:,self.offset:self.offset+image_token_num]=float("-inf")
                else:
                    attn_weights[:,:,:,self.offset:self.offset+image_token_num]=float("-inf")
        
        if self.attncut:
            if attn_weights.shape[2]>image_token_num:
                mask = attn_weights.clone()
                mask[:,:,self.offset:self.offset+image_token_num,self.offset:self.offset+image_token_num]=float("-inf")
                for i in range(self.offset, self.offset+image_token_num):
                    mask[:,:,i,i]=0
                attn_weights = attn_weights + mask 
                
        attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
        
        if self.headcut and self.layer_idx>=2:
                mask = torch.load("headmask14.pth").detach().cpu().numpy()
                mask = torch.tensor(mask).cuda()
                mask = mask[self.layer_idx].unsqueeze(1).unsqueeze(1).unsqueeze(0)
                attn_weights[:,:,:,self.offset:self.offset+image_token_num]= attn_weights[:,:,:,15:15+image_token_num] * mask
        ...
        if attn_weights.shape[2]==1:
            ...
            torch.save(res, 'headcut_mask/'+str(self.layer_idx)+'.pth')
